...
- (void)setInitialSyncCompleted {
    ...
    [[NSUserDefaults standardUserDefaults] synchronize];
          NSLog(@"Sync 6 : complete");
}

- (NSDate *)mostRecentUpdatedAtDateForEntityWithName:(NSString *)entityName {
    __block NSDate *date = nil;
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:entityName];
    NSLog(@"Sync date with entity name 1 : complete");
    [request setSortDescriptors:[NSArray arrayWithObject:
                                 [NSSortDescriptor sortDescriptorWithKey:@"updatedAt" ascending:NO]]];
    [request setFetchLimit:1];
     NSLog(@"Sync date with entity name 2 : complete");
    [[[SDCoreDataController sharedInstance] backgroundManagedObjectContext] performBlockAndWait:^{
     ...
     NSLog(@"Sync date with entity name 4 : complete");
    return date;
      NSLog(@"Sync 7 : complete");
}

- (void)downloadDataForRegisteredObjects:(BOOL)useUpdatedAtDate {
    NSMutableArray *operations = [NSMutableArray array];
     NSLog(@"Sync registered objects 1 : complete");
    for (NSString *className in self.registeredClassesToSync) {
      ...
      NSLog(@"Sync registered objects 2 : complete");
        ...
        NSLog(@"Sync registered objects 4 : complete");
...
- (void)deleteJSONDataRecordsForClassWithName:(NSString *)className {
    ...
    if (!deleted) {
        NSLog(@"Unable to delete JSON Records at %@, reason: %@", url, error);
          ...
          NSLog(@"Sync 21 : complete");
}

- (NSDictionary *)JSONDictionaryForClassWithName:(NSString *)className {
    NSURL *fileURL = [NSURL URLWithString:className relativeToURL:[self JSONDataRecordsDirectory]];
    return [NSDictionary dictionaryWithContentsOfURL:fileURL];
          NSLog(@"Sync 22 : complete");
}
